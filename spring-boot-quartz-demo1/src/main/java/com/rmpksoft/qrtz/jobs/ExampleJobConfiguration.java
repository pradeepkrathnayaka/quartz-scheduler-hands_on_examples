...
public class ExampleJobConfiguration {
	...
	public CronTriggerFactoryBean exampleJob1Trigger() {
		return BatchHelper.cronTriggerFactoryBeanBuilder().cronExpression("0 0/1 * 1/1 * ? *")
				.jobDetailFactoryBean(exampleJob1Schedule()).build();
	}

	@Bean
	public JobDetailFactoryBean exampleJob1Schedule() {
		return BatchHelper.jobDetailFactoryBeanBuilder().job(exampleJob1()).build();
	}

	@Bean
	public Job exampleJob1() {
		return jobBuilderFactory.get(JOB_NAME).start(exampleJob1Step()).build();
	}

	private Step exampleJob1Step() {
		return stepBuilderFactory.get(STEP_NAME).<String, String>chunk(2).reader(exampleJob1Reader())
				.processor(exampleJob1Processor()).writer(exampleJob1Writer()).build();
	}
	...
	public ItemReader<String> exampleJob1Reader() {
		return new ItemReader<String>() {
			...
			private void fetch() {
				...
				this.sampleData = IntStream.range(0, 20).boxed().map(String::valueOf).map(s -> s + "-read")
	...
	private ItemWriter<String> exampleJob1Writer() {
		return items -> items.stream().map(o -> "[" + JOB_NAME + "] " + o + "-write").forEach(System.out::println);
	}
}
